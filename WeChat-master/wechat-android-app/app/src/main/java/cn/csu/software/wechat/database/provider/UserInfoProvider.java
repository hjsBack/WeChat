...
public class UserInfoProvider extends ContentProvider {
    ...
    static {
        mUriMatcher.addURI(UserInfoContent.AUTHORITIES, "/friend_chat_info", CHAT_MESSAGE);
    }
    @Override
    public boolean onCreate() {
        mDatabaseHelper = UserInfoDatabaseHelper.getInstance(getContext(), 1);
        return false;
    }
    ...
    public Cursor query(@NonNull Uri uri, @Nullable String[] projection, @Nullable String selection, @Nullable String[] selectionArgs, @Nullable String sortOrder) {
        ...
        if (mUriMatcher.match(uri) == CHAT_MESSAGE) {
            ...
            cursor = sqLiteDatabase.query(UserInfoContent.TABLE_NAME, projection, selection,
    ...
    public Uri insert(@NonNull Uri uri, @Nullable ContentValues contentValues) {
        ...
        if (mUriMatcher.match(uri) == CHAT_MESSAGE) {
            ...
            long rowId = sqLiteDatabase.insert(UserInfoDatabaseHelper.TABLE_NAME, null, contentValues);
            if (rowId > 0) {
    ...
    public int delete(@NonNull Uri uri, @Nullable String selection, @Nullable String[] selectionArgs) {
        ...
        if (mUriMatcher.match(uri) == CHAT_MESSAGE) {
            SQLiteDatabase sqLiteDatabase = mDatabaseHelper.getWritableDatabase();
            count = sqLiteDatabase.delete(UserInfoContent.TABLE_NAME, selection, selectionArgs);
            sqLiteDatabase.close();
        }
        return count;
    }
